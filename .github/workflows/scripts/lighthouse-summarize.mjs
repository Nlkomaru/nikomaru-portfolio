...
for (const file of files) {
  ...
  if (row.performance < thresholds.performance) failures.push(`${row.url} performance ${row.performance}`)
  if (row.accessibility < thresholds.accessibility) failures.push(`${row.url} accessibility ${row.accessibility}`)
  if (row.bestPractices < thresholds.bestPractices) failures.push(`${row.url} best-practices ${row.bestPractices}`)
...
for (const row of rows) {
  lines.push(`| ${row.url} | ${row.performance} | ${row.accessibility} | ${row.bestPractices} | ${row.seo} |`)
}
lines.push('')
